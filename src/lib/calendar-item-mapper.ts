...
export function taskToCalendarItem(task: Task): CalendarItem | null {
  ...
  return {
    ...
    relatedType: task.companyId ? "company" : null,
    relatedId: task.companyId ?? null,
    relatedName: task.companyName ?? null,
...
export function eventToCalendarItem(event: CalendarEvent): CalendarItem {
  return {
    id: `event-${event.id}`,
    itemType: event.eventType === "meeting" ? "meeting" : event.eventType === "sales" ? "appointment" : "event",
    title: event.title,
    ...
    relatedContactName: event.relatedContactName,
