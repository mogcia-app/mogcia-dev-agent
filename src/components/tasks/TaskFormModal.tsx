...
export function TaskFormModal({ currentMember, members, companies, products, projects, initialValues, onClose, onSubmit }: { currentMember: MemberOption; members: MemberOption[]; companies: CompanyOption[]; products: ProductOption[]; projects: ProjectOption[]; initialValues?: Partial<TaskDraft>; onClose: () => void; onSubmit: (draft: TaskDraft) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#1F1F22]/25 p-4 backdrop-blur-sm">
      <section className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-2xl">
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button className="h-11 rounded-xl border border-[#E2E8F0] px-5 text-sm font-medium text-[#475569]" onClick={onClose} type="button">キャンセル</button>
          <button className="h-11 rounded-xl bg-[#D47A95] px-6 text-sm font-medium text-white disabled:opacity-50" disabled={saving || !draft.title.trim()} onClick={() => void save()} type="button">保存</button>
        </div>
      </section>
    </div>
  );
