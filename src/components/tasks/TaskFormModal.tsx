...
export function TaskFormModal({ currentMember, members, companies, onClose, onSubmit }: { currentMember: MemberOption; members: MemberOption[]; companies: CompanyOption[]; onClose: () => void; onSubmit: (draft: TaskDraft) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-[#1F1F22]/25 p-4 backdrop-blur-sm">
      <section className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-none border border-[#F0DEE2] bg-white p-5 shadow-2xl">
        ...
        <TaskFormFields canAssign companies={companies} currentUserId={currentMember.id} draft={draft} members={members} onChange={setDraft} readOnly={false} />
