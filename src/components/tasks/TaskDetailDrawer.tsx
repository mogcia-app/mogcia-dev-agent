    ...
    <div className="fixed inset-0 z-50 bg-[#1F1F22]/20 backdrop-blur-sm">
      <aside className="ml-auto h-full w-full max-w-2xl overflow-auto border-l border-[#F0DEE2] bg-white p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-[#EC6F8B]">タスク詳細</p>
        ...
        <div className="mt-6 flex flex-wrap justify-between gap-3">
          <div className="flex gap-2">
            <button className="inline-flex h-11 items-center gap-2 rounded-none border border-[#F0DEE2] px-4 text-sm font-medium text-[#6F676B]" onClick={() => void onDuplicate(task)} type="button"><Copy className="h-4 w-4" />複製</button>
            {canDelete ? <button className="inline-flex h-11 items-center gap-2 rounded-none border border-[#F6CBD2] px-4 text-sm font-medium text-[#E65A78]" onClick={() => void remove()} type="button"><Trash2 className="h-4 w-4" />削除</button> : null}
          </div>
          <div className="flex gap-2">
            {editing ? <><button className="h-11 rounded-lg border border-[#F0DEE2] px-5 text-sm font-medium text-[#6F676B]" onClick={() => { setDraft(taskToDraft(task)); setEditing(false); }} type="button">キャンセル</button><button className="h-11 rounded-lg bg-[#EC6F8B] px-6 text-sm font-medium text-white disabled:opacity-50" disabled={!canEdit || saving || !draft.title.trim()} onClick={() => void save()} type="button">保存</button></> : <><button className="inline-flex h-11 items-center gap-2 rounded-lg border border-[#F0DEE2] px-5 text-sm font-medium text-[#6F676B]" disabled={!canEdit} onClick={() => setEditing(true)} type="button"><Pencil className="h-4 w-4" />編集</button><button className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#EC6F8B] px-5 text-sm font-medium text-white disabled:opacity-50" onClick={() => void onToggle(task, true)} disabled={!canEdit} type="button"><CheckCircle2 className="h-4 w-4" />完了にする</button></>}
          </div>
        </div>
      </aside>
    </div>
