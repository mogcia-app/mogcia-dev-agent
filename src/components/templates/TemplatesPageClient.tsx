  ...
  return (
    <section>
      <PageHeader
        ...
        actions={<button className="inline-flex h-10 items-center gap-2 rounded-lg bg-[#EC6F8B] px-4 text-sm font-medium text-white" onClick={openCreate} type="button"><Plus className="h-4 w-4" />テンプレートを追加</button>}
      />
      ...
      <section className="mt-5 rounded-xl border border-[#E5E7EB] bg-white p-4 sm:p-5">
        <div className="grid gap-3 border-b border-[#E5E7EB] pb-4 md:grid-cols-[minmax(0,1fr)_180px_200px]">
          <label className="flex h-10 items-center gap-2 rounded-lg border border-[#E5E7EB] px-3 text-sm text-[#777]"><Search className="h-4 w-4" /><input className="min-w-0 flex-1 bg-transparent outline-none" placeholder="テンプレート名・本文で検索" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
...
function TemplateRow({ template, onOpen, onCopy, onEdit }: { template: BusinessTemplate; onOpen: () => void; onCopy: () => void; onEdit: () => void }) {
  return (
    <article className="grid gap-3 border-b border-[#E5E7EB] px-4 py-3 last:border-b-0 md:grid-cols-[minmax(0,1fr)_150px_auto] md:items-center">
      <button className="min-w-0 text-left" onClick={onOpen} type="button"><span className="flex items-center gap-2"><span className="truncate text-sm font-semibold text-[#2B2B2B]">{template.title}</span>{template.favorite ? <span className="shrink-0 text-xs font-medium text-[#EC6F8B]">よく使う</span> : null}</span><span className="mt-1 block truncate text-xs text-[#8A8186]">{template.description || template.subject || template.content}</span></button>
      <span className="w-fit rounded-md bg-[#FFF2F5] px-2 py-1 text-xs font-medium text-[#EC6F8B]">{categoryLabels[template.category]}</span>
      <div className="flex flex-wrap gap-2 md:justify-end"><button className="h-8 rounded-md bg-[#EC6F8B] px-3 text-xs font-medium text-white" onClick={onCopy} type="button">コピー</button><button className="h-8 rounded-md border border-[#E5E7EB] px-3 text-xs text-[#4B5563]" onClick={onOpen} type="button">内容を見る</button><button className="h-8 rounded-md border border-[#E5E7EB] px-3 text-xs text-[#4B5563]" onClick={onEdit} type="button">編集</button></div>
    </article>
  );
}
      ...
      <div className="grid gap-5">
        <div>
          <div className="flex items-start justify-between gap-3">
            ...
            <button className="h-9 rounded-lg border border-[#F0E7E9] px-3 text-xs font-medium text-[#EC6F8B]" onClick={onFavorite} type="button">{template.favorite ? "よく使うから外す" : "よく使うに追加"}</button>
          </div>
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-[#F0E7E9] pt-4">
          ...
          <button className="h-10 rounded-lg bg-[#EC6F8B] px-4 text-sm font-medium text-white" onClick={onCopy} type="button">本文をコピー</button>
        </div>
      </div>
    </Drawer>
  );
