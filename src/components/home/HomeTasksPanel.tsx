...
export function HomeTasksPanel({ initialTaskId, initialCreateOpen = false, store, options }: { initialTaskId?: string; initialCreateOpen?: boolean; store: ReturnType<typeof useTasks>; options: ReturnType<typeof useWorkspaceOptions> }) {
  ...
  return <section className="rounded-xl border border-[#E8E3E1] bg-white p-5 shadow-[0_8px_24px_rgba(31,31,34,0.03)]" id="tasks">
    <div className="flex items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-semibold text-[#25242A]"><ListChecks className="h-5 w-5 text-[#EC6F8B]" />次にやること（タスク）</h2><p className="mt-1 text-xs leading-5 text-[#8A8186]">実際に対応する一つひとつの作業です。完了したらチェックします。</p></div>{filtered.length > 5 ? <button className="shrink-0 text-xs font-medium text-[#EC6F8B]" onClick={() => setExpanded((value) => !value)} type="button">{expanded ? "閉じる" : "すべて見る →"}</button> : null}</div>
    ...
    {message ? <p className="mt-3 text-xs text-[#C44B63]" role="status">{message}</p> : null}{store.error ? <p className="mt-3 text-xs text-red-600" role="alert">{store.error}</p> : null}
    {store.loading ? <div className="mt-4 grid gap-2">{Array.from({ length: 4 }).map((_, index) => <div className="h-10 animate-pulse bg-[#F7F5F5]" key={index} />)}</div> : visible.length ? <div className="mt-3 divide-y divide-[#F0E7E9] border-y border-[#F0E7E9]">{visible.map((task) => <TaskRow canEdit={canManage(task)} key={task.id} onOpen={() => setSelectedId(task.id)} onToggle={() => toggle(task)} task={task} />)}</div> : <p className="mt-4 py-5 text-center text-sm text-[#8A8186]">{view === "due" ? "期限が近いタスクはありません" : "未完了のタスクはありません"}</p>}
    <button className="mt-4 inline-flex h-9 items-center gap-1 border border-[#F7CAD2] px-3 text-sm font-medium text-[#EC6F8B]" onClick={() => setCreateOpen(true)} type="button"><Plus className="h-4 w-4" />タスクを追加</button>
