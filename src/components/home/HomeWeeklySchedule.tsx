...
export function HomeWeeklySchedule({ createOpen, onCreateClose, onCreateOpen }: { createOpen: boolean; onCreateClose: () => void; onCreateOpen: () => void }) {
  ...
  return <section className="rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-[0_8px_24px_rgba(31,31,34,0.03)] sm:p-5" id="schedule">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="flex items-center gap-2 text-base font-semibold text-[#25242A]"><CalendarDays className="h-5 w-5 text-[#D47A95]" />今週のスケジュール</h2><p className="mt-1 text-sm text-[#64748B]">{rangeLabel(days)}</p></div><div className="flex flex-wrap items-center gap-2"><button aria-label="前の週" className="grid h-9 w-9 place-items-center border border-[#E5E7EB]" onClick={() => setStart(addDays(start, -7))} type="button"><ChevronLeft className="h-4 w-4" /></button><button aria-label="次の週" className="grid h-9 w-9 place-items-center border border-[#E5E7EB]" onClick={() => setStart(addDays(start, 7))} type="button"><ChevronRight className="h-4 w-4" /></button><button className="h-9 border border-[#E5E7EB] px-3 text-xs font-medium" onClick={() => setStart(startOfCurrentWeek())} type="button">今週</button><Link className="ml-2 text-sm font-medium text-[#D47A95]" href={"/calendar" as Route}>カレンダーを開く →</Link></div></div>
    {notice ? <p className="mt-3 text-xs text-[#9B4862]" role="status">{notice}</p> : null}{calendar.error ? <p className="mt-3 text-xs text-red-600">{calendar.error}</p> : null}
    {calendar.loading ? <div className="mt-4 h-72 animate-pulse rounded-lg bg-[#F7F5F5]" /> : <>
      <div className="mt-4 hidden max-h-[460px] overflow-auto overscroll-contain rounded-lg border border-[#E5E7EB] md:block"><div className="min-w-[860px]"><div className="sticky top-0 z-30 grid grid-cols-[58px_repeat(7,minmax(0,1fr))] bg-[#FCFBFA] shadow-[0_1px_0_#DDE1E6]"><div className="border-r border-[#E5E7EB]" />{days.map((day) => <div className={`border-r border-[#E5E7EB] py-2.5 text-center text-sm last:border-r-0 ${isSameCalendarDate(day, new Date()) ? "bg-[#FDF0F4] font-semibold text-[#D47A95]" : "text-[#4F5665]"}`} key={day.toISOString()}>{day.getDate()} <span className="text-xs">{day.toLocaleDateString("ja-JP", { weekday: "short" })}</span></div>)}</div>{weekItems.some((item) => item.allDay) ? <div className="grid grid-cols-[58px_repeat(7,minmax(0,1fr))] border-t border-[#E5E7EB]"><div className="border-r border-[#E5E7EB] px-1 py-2 text-center text-[10px] text-[#94A3B8]">終日</div>{days.map((day) => <div className="min-h-10 border-r border-[#E5E7EB] p-1 last:border-r-0" key={day.toISOString()}>{itemsForDate(weekItems, day).filter((item) => item.allDay).map((item) => <button className="block w-full truncate rounded-md bg-[#FDF0F4] px-1.5 py-1 text-left text-[10px] font-medium text-[#9B4862]" key={item.id} onClick={() => setSelected(item)} type="button">{item.title}</button>)}</div>)}</div> : null}<div className="grid grid-cols-[58px_repeat(7,minmax(0,1fr))] border-t border-[#E5E7EB]"><div className="relative border-r border-[#E5E7EB]" style={{ height: hours.length * 60 }}>{hours.map((hour, index) => <span className="absolute right-2 -translate-y-1/2 text-[10px] font-medium text-[#8A919F]" key={hour} style={{ top: index * 60 }}>{String(hour).padStart(2, "0")}:00</span>)}</div>{days.map((day) => <DayColumn day={day} firstHour={firstHour} height={hours.length * 60} items={itemsForDate(weekItems, day)} key={day.toISOString()} onOpen={setSelected} />)}</div></div></div>
      <div className="mt-4 grid gap-4 md:hidden">{days.map((day) => { const items = itemsForDate(weekItems, day); return <div key={day.toISOString()}><h3 className={`border-b pb-2 text-sm font-medium ${isSameCalendarDate(day, new Date()) ? "border-[#F1C2D0] text-[#D47A95]" : "border-[#E5E7EB] text-[#4B5563]"}`}>{day.toLocaleDateString("ja-JP", { month: "numeric", day: "numeric", weekday: "short" })}</h3>{items.length ? <div className="divide-y divide-[#E2E8F0]">{items.map((item) => <button className="flex w-full gap-3 py-3 text-left" key={item.id} onClick={() => setSelected(item)} type="button"><span className="w-24 shrink-0 text-xs text-[#64748B]">{item.allDay ? "終日" : `${formatTime(item.startAt)} - ${formatTime(item.endAt ?? item.startAt)}`}</span><span className="min-w-0 truncate text-sm font-medium text-[#111827]">{item.title}</span></button>)}</div> : <p className="py-3 text-xs text-[#94A3B8]">予定なし</p>}</div>; })}</div>
    </>}
    <button className="mt-4 inline-flex h-9 items-center gap-1 border border-[#F1C2D0] px-3 text-sm text-[#D47A95] md:hidden" onClick={onCreateOpen} type="button"><Plus className="h-4 w-4" />予定を追加</button>
