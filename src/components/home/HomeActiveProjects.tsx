...
export function HomeActiveProjects({ projects, companies, products, loading }: { projects: ProjectOption[]; companies: CompanyOption[]; products: ProductOption[]; loading: boolean }) {
  ...
  return <section className="overflow-hidden rounded-2xl border border-[#E2E8F0] bg-white shadow-[0_10px_32px_rgba(31,31,34,0.04)]">
    <header className="flex flex-col gap-4 border-b border-[#E5E7EB] px-5 py-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      <div className="flex items-center gap-4"><span className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#FDF0F4] text-[#D47A95]"><Folder className="h-6 w-6" /></span><div><h2 className="text-xl font-semibold text-[#171923]">進行中の仕事（プロジェクト）</h2><p className="mt-1 text-sm text-[#7A8190]">複数のタスクをまとめる仕事の単位です。</p></div></div>
      <div className="flex items-center justify-end gap-4">{activeProjects.length > 6 ? <button className="inline-flex h-10 items-center gap-1 text-sm font-medium text-[#252A35]" onClick={() => setExpanded((value) => !value)} type="button">{expanded ? "閉じる" : "すべて見る"}<ChevronRight className={`h-4 w-4 transition ${expanded ? "rotate-90" : ""}`} /></button> : null}<button className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#D47A95] px-5 text-sm font-medium text-white shadow-none hover:bg-[#EB527B]" onClick={() => setCreateOpen(true)} type="button"><Plus className="h-4 w-4" />プロジェクトを追加</button></div>
    </header>
    {loading ? <div className="grid gap-2 p-6">{Array.from({ length: 4 }).map((_, index) => <div className="h-14 animate-pulse rounded-lg bg-[#F7F5F5]" key={index} />)}</div> : rows.length ? <div><div className="grid grid-cols-[minmax(150px,.8fr)_minmax(220px,1.4fr)_minmax(160px,1fr)] gap-4 bg-[#FCFAFB] px-5 py-3 text-xs font-medium text-[#697184]"><span>会社 / 関連先</span><span>仕事のまとまり</span><span>現在の段階</span></div><div className="divide-y divide-[#E5E7EB]">{rows.map((project) => <ProjectRow key={project.id} project={project} />)}</div></div> : <div className="px-6 py-12 text-center"><Folder className="mx-auto h-9 w-9 text-[#D7C8CD]" /><p className="mt-3 text-sm text-[#7A8190]">現在進行中のプロジェクトはありません</p></div>}
...
export function CreateProjectModal({ companies, products, onClose }: { companies: CompanyOption[]; products: ProductOption[]; onClose: () => void }) {
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [draft, setDraft] = useState<ProjectDraft>({ name: "", type: "client", status: "active", phase: "", currentPosition: "", nextAction: "", notes: "", companyId: "", companyName: "", productId: "", productName: "", description: "", startDate: "", targetDate: "" });
