...
export function KnowledgeWorkspace({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  const controls = <div className="space-y-3"><div className="grid grid-cols-2 gap-2"><button className="h-8 rounded-md bg-[#EC6F8B] px-2 text-xs font-medium text-white" onClick={() => setPathOpen(true)} type="button">パスから作成</button><button className="h-8 rounded-md border border-[#E5E7EB] bg-white px-2 text-xs" onClick={() => openImport(false)} type="button">treeを貼り付け</button><button className="inline-flex h-8 items-center justify-center gap-1 rounded-md border border-[#E5E7EB] bg-white px-2 text-xs" onClick={() => void create("folder", null)} type="button"><FolderPlus className="h-3.5 w-3.5" />フォルダ</button><button className="inline-flex h-8 items-center justify-center gap-1 rounded-md border border-[#E5E7EB] bg-white px-2 text-xs" onClick={() => void create("document", null)} type="button"><FilePlus2 className="h-3.5 w-3.5" />ページ</button></div><div className="flex items-center gap-2 rounded-md border border-[#E5E7EB] bg-white px-2"><Search className="h-3.5 w-3.5 text-[#8A8186]" /><input aria-label="ドキュメントとツリーを検索" className="h-9 min-w-0 flex-1 bg-transparent text-xs outline-none" onChange={(event) => setQuery(event.target.value)} placeholder="名前・パス・本文を検索" value={query} />{query ? <button aria-label="検索をクリア" onClick={() => setQuery("")} type="button"><X className="h-3.5 w-3.5" /></button> : null}</div>{query ? <div className="max-h-64 overflow-y-auto rounded-md border border-[#E5E7EB] bg-white p-1">{search.results.length ? search.results.map((node) => <button className="flex w-full items-start gap-2 rounded px-2 py-2 text-left text-xs hover:bg-[#FFF0F3]" key={node.id} onClick={() => { setSelectedId(node.id); setQuery(""); }} type="button">{node.type === "folder" ? <FolderPlus className="mt-0.5 h-3.5 w-3.5 shrink-0" /> : <FilePlus2 className="mt-0.5 h-3.5 w-3.5 shrink-0" />}<span className="min-w-0"><span className="block truncate font-medium">{node.title}</span><span className="mt-0.5 block truncate text-[11px] text-[#8A8186]">{search.paths.get(node.id)}</span>{node.type === "document" && matchingExcerpt(node.content, query) ? <span className="mt-1 line-clamp-2 block text-[11px] leading-4 text-[#6F676B]">{matchingExcerpt(node.content, query)}</span> : null}</span></button>) : <p className="px-2 py-2 text-xs text-[#8A8186]">名前・パス・本文に一致する項目がありません。</p>}</div> : null}</div>;
  ...
  return <section className="min-h-[calc(100vh-120px)]">
    ...
    <div className="mt-5 grid min-h-[72vh] min-w-0 overflow-hidden rounded-md border border-[#D8D8D8] bg-white lg:grid-cols-[290px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]"><aside className="hidden min-w-0 overflow-y-auto border-r border-[#D8D8D8] bg-white lg:block"><div className="sticky top-0 z-10 border-b border-[#D8D8D8] bg-white p-3">{controls}</div><div className="p-3">{tree}</div></aside><main className="min-w-0 overflow-y-auto bg-white">{loading ? <p className="p-8 text-sm text-[#8A8186]">読み込み中...</p> : <KnowledgeDocument key={`${selected?.id ?? "none"}:${selected?.updatedAt ?? ""}`} node={selected} onSave={saveDocument} saving={saving} />}</main></div>
