  ...
  return (
    <section>
      <PageHeader
        ...
        actions={
          <Link className="inline-flex h-11 items-center gap-2 rounded-xl bg-white px-5 text-sm font-medium text-[#475569] shadow-none ring-1 ring-[#E2E8F0]" href={"/settings/desktop" as Route}>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="space-y-5">
          <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-base font-medium text-[#111827]">新しい開発依頼</h2>
                <p className="mt-1 text-sm font-semibold text-[#64748B]">コード修正、API実装、調査などの開発作業だけを受け付けます。</p>
              </div>
            ...
            <div className="mt-4 rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-3">
              <textarea
                className="min-h-28 w-full resize-y bg-transparent text-base font-semibold leading-7 text-[#111827] outline-none placeholder:text-[#B7B0B3]"
                onChange={(event) => setMessage(event.target.value)}
                placeholder="開発Agentに依頼したい内容を入力"
                value={message}
              />
              <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-[#E2E8F0] pt-3">
                <div className="flex flex-wrap gap-2">
                  {exampleRequests.map((example) => (
                    <button className="h-8 rounded-xl bg-white px-3 text-xs font-medium text-[#475569] ring-1 ring-[#E2E8F0]" key={example} onClick={() => setMessage(example)} type="button">{example}</button>
                  ))}
                </div>
                <button className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#D47A95] px-5 text-sm font-medium text-white disabled:opacity-50" disabled={!message.trim() || savingRequest} onClick={() => void submitRequest()} type="button">
        ...
        <aside className="space-y-5">
          <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <h2 className="flex items-center gap-2 text-base font-medium text-[#111827]"><Bell className="h-5 w-5 text-[#D47A95]" />通知</h2>
              <div className="flex items-center gap-2">
                <span className="text-xs font-medium text-[#64748B]">未読 {unreadNotificationCount} / 完了 {completedNotificationCount}</span>
                <button className="inline-flex h-8 items-center gap-1 rounded-xl bg-[#FDF0F4] px-3 text-xs font-medium text-[#D47A95]" onClick={() => void updateNotification({ action: "mark_all_read" })} type="button">
                  <CheckCircle2 className="h-3.5 w-3.5" />
                  すべて既読
                </button>
                <button className="inline-flex h-8 items-center gap-1 rounded-xl bg-white px-3 text-xs font-medium text-[#9B4862] ring-1 ring-[#E2E8F0]" onClick={() => void deleteNotifications()} type="button">
            ...
            <div className="mt-4 grid gap-2">
              {visibleNotifications.slice(0, 8).map((notification) => (
                <div className="rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-3 text-sm font-semibold text-[#475569]" key={notification.id}>
                  <Link href={(notification.targetUrl || "/agent") as Route}>
                    <span className="block font-medium text-[#111827]">{notification.title}</span>
                    <span className="mt-1 block">{notification.message}</span>
                  </Link>
                  <div className="mt-3 flex flex-wrap gap-2">
                    <button className="h-8 rounded-xl bg-white px-3 text-xs font-medium text-[#475569] ring-1 ring-[#E2E8F0]" onClick={() => void updateNotification({ notificationId: notification.id, read: true })} type="button">既読</button>
                    <button className="h-8 rounded-xl bg-white px-3 text-xs font-medium text-[#475569] ring-1 ring-[#E2E8F0]" onClick={() => void updateNotification({ notificationId: notification.id, completed: !notification.completed, read: true })} type="button">{notification.completed ? "未完了" : "完了"}</button>
                    <button className="h-8 rounded-xl bg-white px-3 text-xs font-medium text-[#9B4862] ring-1 ring-[#E2E8F0]" onClick={() => void deleteNotifications([notification.id])} type="button">削除</button>
                  </div>
                </div>
              ))}
              {visibleNotifications.length === 0 ? <p className="rounded-xl border border-dashed border-[#E2E8F0] bg-[#FFFFFF] p-5 text-center text-sm font-medium text-[#64748B]">通知はまだありません。</p> : null}
            </div>
          </section>
  ...
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-4 shadow-none">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="grid h-9 w-9 place-items-center rounded-xl bg-[#FDF0F4] text-[#D47A95]">{icon}</span>
      </div>
...
function RecentRuns({ loading, runs }: { loading: boolean; runs: AgentRun[] }) {
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
      <h2 className="text-base font-medium text-[#111827]">最近の実行履歴</h2>
      <div className="mt-4">
...
function RunRow({ run }: { run: AgentRun }) {
  return (
    <Link className="grid gap-3 rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-4 transition hover:border-[#F1C2D0] md:grid-cols-[1fr_auto]" href={`/agent?runId=${run.id}` as Route}>
      <span className="min-w-0">
        <span className="flex flex-wrap items-center gap-2">
          <StatusBadge status={run.status} />
          <span className="text-xs font-medium text-[#64748B]">{sourceLabel(run.source)}</span>
        </span>
        <span className="mt-2 block truncate text-base font-semibold text-[#111827]">{run.title}</span>
      </span>
      <span className="flex items-center gap-3 text-sm font-medium text-[#D47A95]">
        {run.progress ?? 0}%
...
function AgentRunDetail({
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div className="min-w-0">
            <Link className="text-sm font-medium text-[#D47A95]" href={"/agent" as Route}>一覧へ戻る</Link>
            <h2 className="mt-2 truncate text-base font-semibold text-[#111827]">{run.title}</h2>
            <p className="mt-2 text-sm font-semibold text-[#64748B]">Run ID: {run.id}</p>
          </div>
          <StatusBadge status={run.status} />
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-4">
          <Panel title="元の依頼" icon={<MessageSquarePlus className="h-5 w-5" />}>
            <p className="whitespace-pre-wrap text-sm font-semibold leading-7 text-[#111827]">{request?.rawMessage || "元の依頼を取得中です。"}</p>
          </Panel>
          <Panel title="Agent回答" icon={<Sparkles className="h-5 w-5" />}>
            {run.answer ? <p className="whitespace-pre-wrap text-sm font-semibold leading-7 text-[#111827]">{run.answer}</p> : <EmptyInline text="回答はまだありません。" />}
            {run.pendingAction ? (
              <div className="mt-4 rounded-xl border border-[#F4D5AC] bg-[#FFF8E8] p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <button className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#D47A95] px-4 text-sm font-medium text-white disabled:opacity-50" disabled={approving} onClick={() => onApprove(run.id, "approve")} type="button">
                    {approving ? <LoadingSpinner label="実行中" /> : <CheckCircle2 className="h-4 w-4" />}
                    実行する
                  </button>
                  <button className="inline-flex h-10 items-center gap-2 rounded-xl bg-white px-4 text-sm font-medium text-[#475569] ring-1 ring-[#E2E8F0] disabled:opacity-50" disabled={approving} onClick={() => onApprove(run.id, "cancel")} type="button">
            ...
            {run.pendingSelection ? (
              <div className="mt-4 rounded-xl border border-[#D9E7F7] bg-[#F3F8FF] p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {run.pendingSelection.candidates.map((candidate) => (
                    <button className="grid gap-1 rounded-xl border border-[#CFE0F3] bg-white p-3 text-left transition hover:border-[#9DBFE4] disabled:opacity-50" disabled={!candidate.id || selectingCandidateId === candidate.id} key={candidate.id ?? candidate.title} onClick={() => candidate.id ? onSelectCandidate(run.id, candidate.id) : undefined} type="button">
                      <span className="text-sm font-semibold text-[#111827]">{candidate.title}</span>
                      {candidate.subtitle ? <span className="text-xs font-medium text-[#6F7F8F]">{candidate.subtitle}</span> : null}
                      {selectingCandidateId === candidate.id ? <span className="text-xs font-medium text-[#D47A95]">選択中...</span> : null}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <Panel title="ログ" icon={<FileText className="h-5 w-5" />}>
            {run.logs?.length ? (
              <div className="grid gap-2">
                {run.logs.map((log, index) => <p className="rounded-xl bg-[#FFFFFF] px-3 py-2 text-sm font-semibold text-[#475569]" key={`${log}-${index}`}>{log}</p>)}
              </div>
            ...
            {run.toolLogs?.length ? (
              <div className="grid gap-2">
                {run.toolLogs.map((log, index) => (
                  <div className="grid gap-1 rounded-xl bg-[#FFFFFF] px-3 py-2 text-sm font-semibold text-[#475569]" key={`${log.toolName}-${index}`}>
                    <span className="font-semibold text-[#111827]">{log.toolName} / {log.status === "success" ? "成功" : "失敗"}</span>
  ...
  return (
    <div className="mt-4 grid gap-3 md:grid-cols-2">
      {cards.map((card, index) => {
        const content = (
          <article className="h-full rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-4 transition hover:border-[#F1C2D0]">
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="truncate text-base font-semibold text-[#111827]">{card.title}</p>
                {card.subtitle ? <p className="mt-1 text-xs font-medium text-[#64748B]">{card.subtitle}</p> : null}
              </div>
              {card.tone && card.tone !== "default" ? <span className="rounded-xl bg-white px-2 py-1 text-xs font-semibold text-[#D47A95] ring-1 ring-[#E2E8F0]">{card.tone}</span> : null}
            </div>
            {card.body ? <p className="mt-3 whitespace-pre-wrap text-sm font-semibold leading-6 text-[#475569]">{card.body}</p> : null}
            {card.meta?.length ? (
              <dl className="mt-3 grid gap-2">
                {card.meta.map((item) => (
                  <div className="grid grid-cols-[88px_1fr] gap-2 text-xs" key={`${card.title}-${item.label}`}>
                    ...
                    <dd className="font-medium text-[#334155]">{item.value}</dd>
                  </div>
                ))}
              </dl>
            ) : null}
...
function Panel({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
      <h3 className="flex items-center gap-2 text-base font-medium text-[#111827]"><span className="text-[#D47A95]">{icon}</span>{title}</h3>
...
function SummaryPanel({ title, value, emptyText }: { title: string; value?: string | null; emptyText: string }) {
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
      ...
      <div className="mt-4">
        {value ? <pre className="max-h-96 overflow-auto whitespace-pre-wrap rounded-xl bg-[#FFFFFF] p-3 text-xs font-semibold leading-5 text-[#334155]">{value}</pre> : <EmptyInline text={emptyText} />}
      </div>
    </section>
  );
}
...
function EmptyInline({ text }: { text: string }) {
  return <p className="rounded-xl border border-dashed border-[#E2E8F0] bg-[#FFFFFF] p-5 text-center text-sm font-medium text-[#64748B]">{text}</p>;
}

function StepRow({ step }: { step: AgentRunStep }) {
  return (
    <div className="grid gap-2 rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-3 md:grid-cols-[120px_120px_1fr]">
      <span className="text-sm font-semibold text-[#111827]">{stepLabel(step.type)}</span>
      <span className="text-sm font-medium text-[#D47A95]">{stepStatusLabel(step.status)}</span>
      <span className="text-sm font-semibold text-[#475569]">{step.message || "詳細はまだありません。"}</span>
    </div>
  );
}
...
function ProjectPanel({
  ...
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-none">
      <h2 className="flex items-center gap-2 text-base font-medium text-[#111827]"><FolderKanban className="h-5 w-5 text-[#D47A95]" />開発プロジェクト</h2>
      <div className="mt-4 grid gap-2">
        {projects.slice(0, 5).map((project) => (
          <div className="rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-3 text-left" key={project.id}>
            <span className="block text-sm font-semibold text-[#111827]">{project.name}</span>
            <span className="mt-1 block text-xs font-medium text-[#64748B]">{project.slug} / {project.defaultBranch || "main"}</span>
          </div>
        ))}
      ...
      <div className="mt-5 border-t border-[#E2E8F0] pt-5">
        ...
        <div className="mt-3 grid gap-3">
          ...
          <button className="inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-[#D47A95] text-sm font-medium text-white disabled:opacity-50" disabled={savingProject || !draft.name.trim() || !draft.slug.trim()} onClick={() => void onSaveProject()} type="button">
...
function InfoBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-[#FFFFFF] p-3 ring-1 ring-[#E2E8F0]">
      <p className="text-xs font-semibold text-[#64748B]">{label}</p>
...
function StatusBadge({ status }: { status: AgentRunStatus }) {
  const tone = status === "completed" ? "bg-[#F3FAF0] text-[#5E9B61]" : status === "error" ? "bg-[#FDF0F4] text-[#9B4862]" : status === "running" ? "bg-[#EEF5FF] text-[#4E76AA]" : status === "requires_approval" ? "bg-[#FFF8E8] text-[#9B7332]" : "bg-white text-[#475569] ring-1 ring-[#E2E8F0]";
  return <span className={`inline-flex rounded-xl px-2.5 py-1 text-xs font-semibold ${tone}`}>{statusLabels[status]}</span>;
}
