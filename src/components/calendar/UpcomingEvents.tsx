...
export function UpcomingEvents({ items, selectedDate, onOpen, onShowAll }: { items: CalendarItem[]; selectedDate: Date; onOpen: (item: CalendarItem) => void; onShowAll: () => void }) {
  ...
  return (
    <section className="rounded-none border border-[#F0E7E9] bg-white p-5 shadow-[0_12px_28px_rgba(142,91,96,0.06)]">
      ...
      <div className="overflow-hidden rounded-none border border-[#F0E7E9]">
        {upcoming.map((item) => {
          const meta = getCategoryMeta(item.category);
          return (
            <button className="grid w-full grid-cols-[64px_70px_1fr_auto_auto] items-center gap-3 border-b border-[#F0E7E9] bg-white px-3 py-3 text-left last:border-b-0 hover:bg-[#FFFBFC]" key={item.id} onClick={() => onOpen(item)} type="button">
              <span className={`${meta.soft} ${meta.text} rounded-none px-2 py-2 text-center text-xs font-bold`}>{formatDateBadge(item.startAt)}<br />{formatWeekday(item.startAt)}</span>
              ...
              <span className="min-w-0">
                ...
                <span className="mt-1 flex items-center gap-1 truncate text-xs font-semibold text-[#777]">{item.location ? <MapPin className="h-3.5 w-3.5" /> : null}{item.location || item.companyName || (item.assigneeId ? getUserDisplayNameById(item.assigneeId, item.assigneeName) : item.assigneeName) || meta.label}</span>
              </span>
              <span className={`hidden rounded-none px-3 py-1 text-xs font-bold sm:inline ${meta.soft} ${meta.text}`}>{meta.label}</span>
              <ChevronRight className="h-4 w-4 text-[#F47E96]" />
            </button>
          );
        })}
      </div>
