...
const filterItems: Array<{ key: keyof CalendarFilterState; label: string; color: string }> = [
  { key: "mine", label: "マイタスク", color: "bg-[#D47A95]" },
  ...
  { key: "members", label: "他のメンバー", color: "bg-[#D47A95]" }
];
...
export function CalendarFilters({ filters, members, member, onFilterChange, onMemberChange }: { filters: CalendarFilterState; members: MemberOption[]; member: string; onFilterChange: (key: keyof CalendarFilterState, value: boolean) => void; onMemberChange: (member: string) => void }) {
  return (
    <section className="rounded-xl border border-[#E2E8F0] bg-white p-5 shadow-[0_12px_28px_rgba(142,91,96,0.06)]">
      <h3 className="text-sm font-medium text-[#111827]">表示フィルター</h3>
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        {filterItems.map((item) => (
          <label className="flex min-h-11 items-center gap-3 text-sm font-medium text-[#475569]" key={item.key}>
