...
export function CalendarEventDrawer({ item, event, leads, canEdit, canDelete, onClose, onEdit, onDelete }: { item: CalendarItem | null; event: CalendarEvent | null; leads: LeadOption[]; canEdit: boolean; canDelete: boolean; onClose: () => void; onEdit: (event: CalendarEvent) => void; onDelete: (eventId: string) => Promise<void> }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#1F1F22]/20 backdrop-blur-sm">
      <aside className="ml-auto h-full w-full max-w-xl overflow-auto border-l border-[#E2E8F0] bg-white p-5 shadow-2xl">
        <div className="mb-5 flex items-start justify-between gap-4">
          <div>
            <span className={`inline-flex rounded-xl px-3 py-1 text-xs font-medium ${meta.soft} ${meta.text}`}>{meta.label}</span>
            <h2 className="mt-3 text-xl font-medium text-[#111827]">{item.title}</h2>
            <p className="mt-2 text-sm font-medium text-[#64748B]">{formatShortDate(item.startAt)} / {formatTimeRange(item.startAt, item.endAt, item.allDay)}</p>
          </div>
          <button className="grid h-10 w-10 place-items-center rounded-xl hover:bg-[#FDF0F4]" onClick={onClose} type="button" aria-label="閉じる"><X className="h-5 w-5" /></button>
        </div>
        <div className="space-y-4 rounded-xl border border-[#E2E8F0] bg-[#FFFFFF] p-4 text-sm font-semibold text-[#475569]">
          ...
          {item.projectName ? <InfoLink href={null} label="プロジェクト" value={item.projectName} /> : null}
          {item.location ? <Info label="場所" value={item.location} /> : null}
        </div>
        <div className="mt-6 flex flex-wrap justify-between gap-3">
          <div className="flex gap-2">
            {item.meetingUrl ? <a className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#D47A95] px-5 text-sm font-medium text-white" href={item.meetingUrl} rel="noreferrer" target="_blank"><ExternalLink className="h-4 w-4" />オンライン会議を開く</a> : null}
            {event && canEdit ? <button className="inline-flex h-11 items-center gap-2 rounded-xl border border-[#E2E8F0] px-5 text-sm font-medium text-[#655D62]" onClick={() => onEdit(event)} type="button"><Pencil className="h-4 w-4" />編集</button> : null}
          </div>
          {event && canDelete ? <button className="inline-flex h-11 items-center gap-2 rounded-xl border border-[#F1C2D0] px-5 text-sm font-medium text-[#D47A95]" onClick={() => void remove()} type="button"><Trash2 className="h-4 w-4" />削除</button> : null}
        </div>
      </aside>
    </div>
  );
...
function InfoLink({ label, value, href }: { label: string; value: string; href: Route | null }) {
  return <p><span className="mr-3 inline-block min-w-24 text-[#9A8F94]">{label}</span>{href ? <Link className="text-[#D47A95] hover:underline" href={href}>{value} →</Link> : value}</p>;
}
