...
export function CalendarPageClient() {
  ...
  return (
    <div className="">
      ...
      {createOpen ? <CalendarEventFormModal companies={workspaceOptions.companies} currentMember={calendar.currentMember} isAdmin={calendar.isAdmin} leads={workspaceOptions.leads} meetings={workspaceOptions.meetings} members={calendar.members} onClose={() => setCreateOpen(false)} onSubmit={calendar.createEvent} products={workspaceOptions.products} projects={workspaceOptions.projects} /> : null}
      {editingEvent ? <CalendarEventFormModal companies={workspaceOptions.companies} currentMember={calendar.currentMember} initialDraft={eventToDraft(editingEvent)} isAdmin={calendar.isAdmin} leads={workspaceOptions.leads} meetings={workspaceOptions.meetings} members={calendar.members} onClose={() => setEditingEvent(null)} onSubmit={(draft) => calendar.updateEvent(editingEvent.id, draft)} products={workspaceOptions.products} projects={workspaceOptions.projects} /> : null}
      <CalendarEventDrawer canDelete={eventCanDelete} canEdit={eventCanEdit} event={selectedEvent} item={selectedItem?.sourceCollection === "calendarEvents" ? selectedItem : null} leads={workspaceOptions.leads} onClose={() => setSelectedItem(null)} onDelete={calendar.deleteEvent} onEdit={(event) => { setEditingEvent(event); setSelectedItem(null); }} />
    </div>
  );
}
