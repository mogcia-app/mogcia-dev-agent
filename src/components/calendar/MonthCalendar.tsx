  ...
  return (
    <section className="rounded-none border border-[#F0E7E9] bg-white p-4 shadow-[0_12px_28px_rgba(142,91,96,0.06)]">
      <div className="mb-4 flex items-center justify-between gap-3">
        ...
        <h3 className="text-base font-medium text-[#2B2B2B]">{formatMonthTitle(month)}</h3>
        <div className="flex items-center gap-2">
          <button className="h-10 rounded-none bg-white px-4 text-sm font-medium text-[#F47E96] shadow-sm ring-1 ring-[#F0E7E9]" onClick={() => { const today = new Date(); onMonthChange(today); onSelectDate(today); }} type="button">今日</button>
      ...
      <div className="grid grid-cols-7 overflow-hidden rounded-none border border-[#F0E7E9] text-sm font-medium text-[#6E666A]">
