...
export function MonthCalendar({ month, selectedDate, items, onMonthChange, onSelectDate }: { month: Date; selectedDate: Date; items: CalendarItem[]; onMonthChange: (date: Date) => void; onSelectDate: (date: Date) => void }) {
  ...
  return (
    <section className="rounded-lg border border-[#F0E7E9] bg-white p-5 shadow-[0_12px_28px_rgba(142,91,96,0.06)]">
      ...
      <div className="grid grid-cols-7 gap-2 text-center text-sm font-bold text-[#6E666A]">
        ...
        {days.map((day) => <CalendarDayCell date={day} items={itemsForDate(items, day)} key={day.toISOString()} month={month} onSelect={onSelectDate} selectedDate={selectedDate} />)}
      </div>
    </section>
  );
}
