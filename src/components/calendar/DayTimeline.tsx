    ...
    <section className="rounded-none border border-[#F0E7E9] bg-white p-5 shadow-[0_12px_28px_rgba(142,91,96,0.06)]">
      <div className="mb-5 flex flex-col gap-3">
        <div>
          ...
          <p className="mt-1 text-sm font-bold text-[#777]">{formatShortDate(selectedDate)}</p>
        </div>
      ...
      {timedItems.length === 0 && allDayItems.length === 0 ? (
        ...
        <div className={view === "timeline" ? "space-y-3" : "grid gap-3 sm:grid-cols-2"}>
          {timedItems.map((item) => (
            <div className={view === "timeline" ? "grid grid-cols-[48px_10px_minmax(0,1fr)] gap-2" : ""} key={item.id}>
              {view === "timeline" ? (
                <>
                  <p className="pt-3 text-xs font-bold text-[#2B2B2B]">{formatTime(item.startAt)}</p>
