...
export default function DesktopSettingsPage() {
  ...
  return (
    <section className="">
      ...
      {issuedToken ? (
        <div className="mt-5 rounded-2xl border border-[#F1C2D0] bg-white p-5 shadow-sm">
          <div className="flex items-center gap-2 text-[#D47A95]">
            <ShieldCheck className="h-5 w-5" />
            <h2 className="font-medium">デスクトップアクセストークン</h2>
          </div>
          <p className="mt-2 text-sm font-semibold text-[#64748B]">このトークンは今だけ表示されます。CLIまたはデスクトップウィジェットの設定へ登録してください。</p>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[360px_1fr]">
        <section className="rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
          ...
          <button className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-none bg-[#D47A95] text-sm font-medium text-white disabled:opacity-50" disabled={saving || !deviceName.trim()} onClick={() => void createDevice()} type="button">
        ...
        <section className="rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <button className="text-sm font-medium text-[#D47A95]" onClick={() => user && void loadDevices(user)} type="button">再読み込み</button>
          </div>
          ...
          <div className="mt-5 grid gap-3">
            {devices.map((device) => (
              <div className="grid gap-3 rounded-2xl border border-[#E2E8F0] bg-[#FFFFFF] p-4 lg:grid-cols-[1fr_120px_180px_100px]" key={device.id}>
                <div>
                ...
                <button className="inline-flex h-9 items-center justify-center gap-2 rounded-none border border-[#F1C2D0] text-sm font-medium text-[#9B4862] disabled:opacity-40" disabled={device.status === "revoked"} onClick={() => void revokeDevice(device.id)} type="button">
