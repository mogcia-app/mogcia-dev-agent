...
export async function GET(request: Request) {
  try {
    ...
    const data = await withDesktopAudit({ userId: auth.userId, deviceId: auth.device.id }, "analysis_read", async () => {
      ...
      return { analyses: snapshot.docs.map((entry) => { const value = entry.data(); const preparation = value.aiAdvice?.meetingPreparation; return { id: entry.id, companyName: String(value.customerName ?? ""), contactName: String(value.contactName ?? ""), productName: String(value.productName ?? ""), salesDomain: value.salesDomain === "meeting" ? "meeting" : "teleapo", status: String(value.aiAdviceStatus ?? value.transcriptionStatus ?? "draft"), summary: String(preparation?.prospectScore?.reason ?? value.memo ?? value.meetingMemo ?? ""), score: typeof preparation?.prospectScore?.score === "number" ? preparation.prospectScore.score : null, rank: String(preparation?.prospectScore?.rank ?? ""), recordedAt: timestampToIso(value.recordedAt), updatedAt: timestampToIso(value.updatedAt), targetURL: value.companyId ? `/sales/companies?id=${encodeURIComponent(String(value.companyId))}&tab=meetings` : "/sales/companies" }; }) };
    });
