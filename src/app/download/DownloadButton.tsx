...
export function DownloadButton() {
  ...
  if (!signedIn) return <Link className="mt-8 inline-flex min-h-14 items-center justify-center rounded-2xl bg-[#D47A95] px-8 font-semibold text-white" href="/">社員ログインしてダウンロード</Link>;
  return <><button className="mt-8 min-h-14 rounded-2xl bg-[#D47A95] px-8 font-semibold text-white shadow-[0_12px_28px_rgba(236,111,139,0.28)] disabled:opacity-60" disabled={downloading} onClick={() => void download()}>{downloading ? "ダウンロード中…" : "Mac版をダウンロード"}</button>{message ? <p className="mt-3 text-sm text-red-600">{message}</p> : null}</>;
}
